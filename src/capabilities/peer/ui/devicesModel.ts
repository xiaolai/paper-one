    ...
    try {
      ...
      offs.push(
        port.onPairingResult((lastResult) => {
          ...
          /* ⚠️ **A RESULT THAT IS NOT ABOUT THE ATTEMPT ON SCREEN MUST NOT
             TOUCH IT, AND SINCE THE PROBE LANDED EVERY PAIRING PRODUCES ONE.**
             `usePairing` has had this filter since the probe was added and this
             surface never got it, though both read the same stream.

             A joiner cannot tell "you never heard me" from "your human is still
             deciding" — this side sends nothing between the two — so it opens a
             second connection beside its first after `PAIR_PROBE_PAUSE`, and
             this side refuses that one `no-pending`. Acted on, that refusal
             cleared `pending` and `sas` five seconds into the comparison: the
             six digits vanished mid-word and the reader was told "Pairing did
             not finish: no-pending" about a pairing that was about to succeed.
             `peer_status.droppedInbound` climbing on this side is the same
             design working as intended.

             `no-pending` is the only reason that is generated by our own
             protocol rather than by anything the reader did, so it is dropped
             by name; anything else naming a DIFFERENT attempt than the one on
             screen is dropped by id. With nothing on screen a stray refusal is
             still shown — somebody dialling with a wrong secret is worth
             seeing. Both rules are `usePairing`'s, deliberately, because the
             two surfaces divide one event stream and a rule that holds on one
             half is not a rule. */
          if (lastResult.reason === 'no-pending') return
          const onScreen = snapshot.pending
          if (onScreen !== null && lastResult.attemptId !== onScreen.attemptId) return
