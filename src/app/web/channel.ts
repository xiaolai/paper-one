import { ServiceCallError, createClient, type CallOptions, type Client } from '../../kernel/core/envelope'

/**
 * The browser's channel to its shelf: the envelope over a WebSocket
 * (phase 18, WI-18.5).
 *
...
export async function connect(options: ConnectOptions = {}): Promise<ShelfChannel> {
  const url = options.url ?? socketUrl(window.location)
