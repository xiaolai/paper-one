...
use super::{accounted, chunks, timings_for, Kokoro, Rendered, WordTiming, FRAME_SAMPLES, SAMPLE_RATE};
...
fn text_of(phonemes: &str, words: &[(usize, usize, usize, usize)]) -> Phonemised {
    Phonemised {
        ...
        words: words
            .iter()
            .map(|(source_start, source_len, phoneme_start, phoneme_len)| SpokenWord {
...
fn a_long_one_breaks_where_a_reader_would() {
    ...
    let rendered: Vec<String> =
        pieces.iter().map(|r| phonemes[r.clone()].iter().collect()).collect();
    assert_eq!(rendered[0], "aaaa.", "the full stop is the best break available");
    assert!(rendered[1].starts_with(' '), "the space begins the next piece, which costs one token");
    assert!(rendered.len() >= 2);
    assert_eq!(rendered.concat(), text, "every phoneme is spoken exactly once");
}
...
fn it_falls_back_to_a_clause_then_a_space_then_wherever_it_must() {
    ...
    assert_eq!(pieces[0], 0..10, "a word longer than the limit is cut at the limit");
    assert_eq!(pieces.iter().map(|r| r.len()).sum::<usize>(), unbroken.len());
}
...
fn a_words_timing_comes_from_the_rounded_durations() {
    ...
    assert_eq!(
        timings,
        vec![
            WordTiming { source_start: 0, source_len: 2, start_ms: ms(4), end_ms: ms(8) },
            WordTiming { source_start: 3, source_len: 2, start_ms: ms(9), end_ms: ms(13) },
        ]
    );
}
...
fn rounding_happens_before_the_sum_and_not_after() {
    ...
    assert_eq!(timings[0].end_ms, ms(13 + 2 + 2), "1.64 and 1.7 round to 2 and 2, not to 3");
}
...
fn a_later_chunks_words_are_placed_after_the_earlier_ones() {
    ...
    let ms = |frames: usize| (frames * FRAME_SAMPLES * 1000 / SAMPLE_RATE as usize) as u32;
    assert_eq!(timings[0].start_ms, ms(101), "the chunks before it take 100 frames");
}
...
fn a_render_with_no_audio_is_refused_rather_than_returned() {
    ...
    let text = format!("{err}");
    assert!(text.contains("no audio"), "the refusal says what happened: {text}");
...
fn durations_that_do_not_account_for_the_samples_are_refused() {
    ...
    let text = format!("{err}");
    assert!(text.contains("1200"), "the refusal shows what the durations account for: {text}");
...
fn the_model_speaks_and_its_durations_account_for_every_sample() {
    let Some(dir) = pack_dir() else {
        panic!("set PAPER_VOICES_PACK to a directory holding model.onnx, tokenizer.json and voices/");
    };
    ...
    assert!(text.unknown.is_empty(), "this sentence is all in the lexicon");

    let out = kokoro.render(&text, "af_heart", 1.0).expect("render");
    // `render` asserts the frame equality itself; this is the audible half.
    assert_eq!(out.sample_rate, SAMPLE_RATE);
    assert!(out.duration_ms() > 1_500, "a nine-word sentence is longer than this");
    assert!(
    ...
    for pair in out.words.windows(2) {
        assert!(pair[0].start_ms <= pair[1].start_ms, "words are spoken in order");
    }
...
fn a_sound_the_model_cannot_say_is_refused_by_name() {
    let Some(dir) = pack_dir() else { panic!("set PAPER_VOICES_PACK") };
    ...
    let text = Phonemised {
        ...
        words: vec![SpokenWord { source_start: 0, source_len: 3, phoneme_start: 0, phoneme_len: 3 }],
        unknown: Vec::new(),
    };
    let err = kokoro.render(&text, "af_heart", 1.0).expect_err("refused");
    assert!(format!("{err}").contains('ʘ'), "the refusal names the sound: {err}");
}
