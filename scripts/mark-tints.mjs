 ...
 * Then paste into `src/kernel/ui/reader/bookCss.ts` and `src/kernel/ui/styles/tokens.css`, which
...
console.log('\n--- src/kernel/ui/reader/bookCss.ts — what the PAINTER is handed ---')
...
console.log('\n--- src/kernel/ui/styles/tokens.css — what the reader SEES, for the swatches ---')
for (const [theme, t] of Object.entries(table)) {
  console.log(`  /* ${theme} */`)
